...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    console.log("PATCH /orders/:id called with body fields:", req.body);
    try {
      const { id } = req.params;
      ...
      // ─── PARSE STYLES DIRECTLY FROM FIELDS ───────────────────────────────
      // Parses inline so every index is captured regardless of whether
      // an `id` field is present (new styles have no id).
      type ParsedStyle = {
        _index: number;
        id?: string;
        styleNo?: string;
      ...
      const stylesMap: Record<number, Omit<ParsedStyle, "_index">> = {};
      for (const key of Object.keys(fields)) {
        if (!key.startsWith("styles[")) continue;
        const match = key.match(/^styles\[(\d+)\]\.(.+)$/);
        if (!match) continue;
        const idx = Number(match[1]);
        const field = match[2];
        if (!stylesMap[idx]) stylesMap[idx] = {};
        stylesMap[idx][field] = fields[key];
      }
      const styles: ParsedStyle[] = Object.keys(stylesMap)
        .map(Number)
        .sort((a, b) => a - b)
        .map((idx) => ({ _index: idx, ...stylesMap[idx] }));
      // ─────────────────────────────────────────────────────────────────────

      // Build pricingProductsMap with safely resolved styleNos
      const pricingProductsMap = await fetchPricingProductsMap(
        styles.map((s) => {
          const existingStyleNo = order.styles?.find(
            (es) => es.id === Number(s.id)
          )?.styleNo;
          return {
            ...s,
            styleNo: s.styleNo ?? existingStyleNo ?? "",
          };
        })
      );
      ...
      console.log("[PATCH] Total styles parsed:", styles.length, styles.map(s => ({ _index: s._index, id: s.id, styleNo: s.styleNo })));

      for (const s of styles) {
        const i = s._index;

        // Determine if this is a new style or an update to an existing one
        const rawId = s.id;
        const styleId =
          rawId !== undefined && rawId !== "" && rawId !== null
            ? Number(rawId)
            : NaN;
        const isNewStyle = Number.isNaN(styleId) || styleId === 0;
        console.log(`[PATCH] style[${i}] => rawId="${rawId}" styleId=${styleId} isNewStyle=${isNewStyle}`);

        let style: Style;

        if (isNewStyle) {
          // ── NEW STYLE: set all NOT NULL columns to safe defaults first ──
          // Without this, style.save() throws a MySQL NOT NULL constraint
          // error for columns like customColor, customSize, comments, etc.
          style = new Style();
          style.order = order;
          style.customColor = "[]";
          style.customSize = "[]";
          ...
          style.comments = "[]";
          style.photoUrls = "[]";
          style.mesh_color = "SAS";
          style.beading_color = "SAS";
          style.lining = "SAS";
          style.lining_color = "SAS";
          style.quantity = 0;
          // Ensure DB NOT NULL / unique `barcode` column doesn't block initial save
          style.barcode = `TEMP-${Date.now()}-${Math.random().toString(36).substring(2,9)}`;
        } else {
          const existing = existingStylesMap.get(styleId);
          // Only 404 for a bad existing-style reference, not for new styles
          if (!existing) {
            return res.status(404).json({
              success: false,
              message: `Order style with id ${styleId} not found`,
            });
          }
          style = existing;
        }

        // Apply only the fields that were actually sent in the payload
        ...
        if (s.sizeCountry !== undefined) {
          style.sizeCountry = sanitizeText(s.sizeCountry) as any;
        }
        if (s.size !== undefined) {
          (style as any).size = sanitizeText(s.size);
        }
        ...
        if (s.liningColor !== undefined) {
          style.lining_color =
            style.lining === "No Lining" ? null : s.liningColor;
        }
        ...
        // Safely resolve styleNo with fallbacks to prevent sanitizeText(undefined)
        const resolvedStyleNo = sanitizeText(
          s.styleNo ?? style.styleNo ?? ""
        ).toLowerCase();
        ...
        applyPricingToStyle(
          style,
          {
            ...
            quantity: s.quantity !== undefined ? Number(s.quantity) : style.quantity,
            customSizesQuantity:
              s.customSizesQuantity ?? style.customSizesQuantity,
          },
          pricingProductsMap.get(resolvedStyleNo),
          order.customer
        );

        await style.save();
        console.log(`[PATCH] style saved => id=${style.id} isNew=${isNewStyle}`);

        // Assign barcode to new styles after first save (id is now available)
        if (isNewStyle) {
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    for (let i = 0; i < styles.length; i++) {
      ...
      if (!styleId) {
        ...
        style.lining_color = "SAS";
        style.quantity = 0;
        // Avoid DB constraint failures for NOT NULL/unique `barcode`
        style.barcode = `TEMP-${Date.now()}-${Math.random().toString(36).substring(2,9)}`;
      }
      ...
      if (s.sizeCountry !== undefined) style.sizeCountry = s.sizeCountry as any;
      if (s.size !== undefined) (style as any).size = s.size;
      if (s.mesh !== undefined) style.mesh_color = s.mesh || "SAS";
      if (s.beading !== undefined) style.beading_color = s.beading || "SAS";
      if (s.lining !== undefined) style.lining = s.lining || "SAS";
      if (s.liningColor !== undefined) {
