...
export const parseCustomSizesQuantity = (
  value: unknown,
): CustomSizeQuantity[] => {
...
export const resolveProductCurrencyPrice = (
  ...
  return {
    amount: toNumber(pricing?.price ?? product?.price),
    currencyId: currency?.id ?? pricing?.currencyId ?? currencyId ?? null,
    currencyCode: currency?.code ?? pricing?.currencyCode ?? "EUR",
    currencySymbol: currency?.symbol ?? pricing?.currencySymbol ?? "\u20ac",
  };
};
